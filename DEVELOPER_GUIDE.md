# Anthropic Mock — Developer Guide

> **TL;DR:** A mock implementation of the Anthropic API for testing
> purposes. This is a base project scaffold — the mock implementation is
> not yet built out.

## 1. Project Overview

A library that mocks the Anthropic API so test suites can exercise
client code without network access or API costs. The toolchain (build,
format, lint, test, CI) is fully wired up; only the mock logic remains
to be implemented.
...
The current code is intentionally a minimal skeleton.

## 2. Tech Stack

| Category                 | Technology       | Notes                                                               |
|--------------------------|------------------|---------------------------------------------------------------------|
| **Runtime**              | Node.js 26       | Required (`engines` field in `package.json`)                        |
| **Package Manager**      | pnpm 11.15.0     | Enforced via `packageManager` field                                 |
| **Language**             | TypeScript       | Strict mode, ES2025 target, ES modules                              |
...
| **Linting**              | oxlint           | Type-aware secondary linter                                         |
| **Structural Rules**     | ast-grep         | Enforces no-function-declaration, no-inline-export, brace stripping |
...
## 4. Project Structure
    ...
    ├── src/
    │   ├── index.ts                          # Mock entry point
    │   └── tests/
    │       └── index.test.ts                 # Skeleton test
...
The project currently has zero runtime dependencies. Add what you need:
